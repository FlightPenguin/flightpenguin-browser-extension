...
var handleFlightResultsReceived = function handleFlightResultsReceived(providerManager, flights, providerName) {
  ...
  var _makeItins = makeItins(flights, existingDepartures, existingItineraries, providerName, windowId, tabId),
      departures = _makeItins.departures,
      itineraries = _makeItins.itins;

  var setSuccessful = providerManager.setItineraries(flightResultsReceived_objectSpread({}, itineraries), existingItinerariesVersion);
  ...
  if (setSuccessful) {
    ...
    var departuresToSend = sortFlights(updatedDepartures, updatedItineraries);
    var nextMessage = {
...
var handleFlightReturnResultsReceived = function handleFlightReturnResultsReceived(providerManager, flights, providerName) {
  ...
  var _makeItins = makeItins(flights, existingDepartures, existingItineraries, providerName, windowId, tabId, true),
...
var southwest_mappings_getUrl_getUrl = function getUrl(formData) {
  var from = formData.from,
      to = formData.to,
      fromDate = formData.fromDate,
      toDate = formData.toDate,
...
var providerURLBaseMap = {
  southwest: southwest_mappings_getUrl_getUrl,
  skyscanner: mappings_getUrl_getUrl,
  expedia: getUrl,
  skiplagged: getUrl_getUrl
};
